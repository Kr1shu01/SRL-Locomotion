...
class x02jumpCfg(LeggedRobotCfg):
    # 定义x02Cfg类，继承自LeggedRobotCfg
    ...
    class env(LeggedRobotCfg.env):
        ...
        single_num_privileged_obs = 64 -10
        num_privileged_obs = int(c_frame_stack * single_num_privileged_obs)
        num_actions = 10
        num_envs = 4096
        episode_length_s = 6.5  # episode length in seconds
    ...
    class noise:
        add_noise = True
        noise_level = 0.6  # 降低噪声水平，以避免跳跃时过多干扰
        class noise_scales:
            # 噪声尺度
            dof_pos = 0.05  # 自由度位置的噪声尺度
            dof_vel = 1.5  # 自由度速度的噪声尺度
            ang_vel = 0.5  # 角速度的噪声尺度   # 0.1
    ...
    class init_state(LeggedRobotCfg.init_state):
        ...
        pos = [0.0, 0.0, 0.7]  # 略低一些的初始位置，以适应跳跃
        default_joint_angles = {
            'L_hip_yaw': 0.,
            'L_hip_roll': 0.,
            'L_hip_pitch': 1.,   # 髋关节初始角度，保持与膝关节相协调的值
            'L_knee_pitch': -2,  # 根据三次多项式调整的膝关节角度
            'L_ankle_pitch': 1,  # 髋关节和膝关节之间的比例关系
            'R_hip_yaw': 0.,
            'R_hip_roll': 0.,
            'R_hip_pitch': 1,   # 右腿的髋关节初始角度
            'R_knee_pitch': -2,  # 右腿的膝关节初始角度
            'R_ankle_pitch': 1,  # 右腿踝关节初始角度
        }
    ...
    class control(LeggedRobotCfg.control):
        ...
        action_scale = 0.25  # 增大动作缩放因子，使跳跃更有力
        decimation = 10
    ...
    class rewards:
        ...
        class scales:
            ...
            # orientation = 4.0
            # torques = -0.001  # 扭矩惩罚比例
            # action_smoothness = -0.01  # 平滑性奖励比例
            # contact = 0.0 #new
            # balance = 0.0 #new
            # collision = -1.0
            # feet_distance = 0.0
...
class x02jumpCfgPPO(LeggedRobotCfgPPO):
    seed = 5
